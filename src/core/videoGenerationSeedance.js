...
/**
 * Submits a video generation task for a single scene to Kie.ai (ByteDance Seedance)
 */
async function generateSeedanceVideo(params) {
  ...
  console.log(`[VideoGen Seedance] Submitting Kie.ai ByteDance Seedance task for Job: ${jobId}, Scene: ${activeSceneId}, talkvid: ${isTalkvid}`);
  ...
  try {
    ...
    console.log(`[VideoGen Seedance] Callback URL: ${callBackUrl}`);

    console.log(`[VideoGen Seedance] Uploading starting frame image to Kie.ai: ${imageResultUrl}`);
    ...
    if (isTalkvid && audio) {
      ...
      console.log(`[VideoGen Seedance] Resolving signed audio URL from S3 key: ${audio}`);
    ...
    console.log(`[VideoGen Seedance] Calling Kie.ai createVeoTask for ${model} with duration: ${resolvedDuration}s...`);
    const taskId = await createVeoTask(model, input, callBackUrl, kieApiKey);
    console.log(`[VideoGen Seedance] Successfully created task ${taskId} for Scene ${activeSceneId}`);

    return taskId;
  } catch (err) {
    console.error(`[VideoGen Seedance] Error triggering video for Job ${jobId}, Scene ${activeSceneId}:`, err);
    throw err;
  }
}
